...
# ==========================================
# שקף 24 (מעודכן): חשיפת הסיליקון - תמונת ה-ILA המלאה
# ==========================================
slide24 = prs.slides.add_slide(prs.slide_layouts[5])
title24 = slide24.shapes.title
title24.text = "Post-Silicon Validation: מסלול הנתונים בסיליקון"
...
add_bullet(tf_text24, "• בניגוד לסימולציה האידיאלית (בשקף 16), כאן אנו דוגמים את החומרה הפיזית פועלת בזמן אמת באמצעות ה-ILA.")
add_bullet(tf_text24, "• השהיית זיכרון (Latency): שימו לב לפער הפיזי - הכתובת 266880 נשלחת (bram_address_reg), אך הנתון 4ea מופיע רק לאחר מכן.")
add_bullet(tf_text24, "• אכיפת שומר הסף: למרות שהאוגר התמלא, רק בקו הצהוב, כש-in_display_area_delayed עולה ל-'1', נתוני ה-RGB נפרצים החוצה (0 -> 4,e,a).")

# תמונה מרכזית וגדולה של ה-ILA
img_path_ila = "docs/presentation/assets/ila_hardware_data_path_transition.png" 
if os.path.exists(img_path_ila):
    slide24.shapes.add_picture(img_path_ila, Inches(0.5), Inches(3.2), width=Inches(9.0))
else:
    img_box_ila = slide24.shapes.add_textbox(Inches(0.5), Inches(3.2), Inches(9.0), Inches(4.0))
    p_ila = img_box_ila.text_frame.paragraphs[0]
    p_ila.text = "[שומר מקום: צילום מסך ILA - ila_hardware_data_path_transition.png]"
    p_ila.font.size = Pt(16); p_ila.font.color.rgb = RGBColor(0, 112, 192); p_ila.alignment = PP_ALIGN.CENTER


# ==========================================
# שקף 25 (מעודכן): סיכום וריפיקציה - אידיאלי מול פיזי
# ==========================================
slide25 = prs.slides.add_slide(prs.slide_layouts[5])
title25 = slide25.shapes.title
title25.text = "סיכום וריפיקציה: סימולציה (ModelSim) מול סיליקון (ILA)"
...
add_bullet(tf_text25, "• המעבר מקופסה שחורה ללבנה: נאלצנו להוסיף אטריביוט mark_debug ל-VHDL כדי להורות לסינתזה לשמר חוטים פנימיים למדידה (White-Box).")
add_bullet(tf_text25, "• הוכחת הפיזיקה: הטבלה מתארת כיצד התיאוריה מהסימולציה תורגמה לתופעות חשמליות/פיזיות בתוך רשת השערים (Netlist) של ה-FPGA.")
...
headers25 = ["האות ב-Data Path", "ModelSim (סביבה וירטואלית)", "Vivado ILA (הסיליקון הפיזי)"]
...
content25 = [
    ["bram_address_reg", "עדכון מתמטי באפס זמן (0ns).", "ניתוב פיזי: 19 חוטים שמפעילים את פורט הקריאה בזיכרון ה-BRAM."],
    ["doutb (תגובת BRAM)", "המידע מופיע מיידית ובקסם.", "הוכחת Latency: חשיפת זמן התגובה האמיתי של בלוק החומרה."],
    ["pxl_data_reg", "השמת משתנה תוכנתית פשוטה.", "יציבות חומרה: הפליפ-פלופים דוגמים את doutb כראוי למרות ההשהיה."],
    ["in_display_area_dly", "דגל בוליאני (True/False).", "פיקוד פיזי (Select Line) שכופה ניתוב על המרבבים (MUX) בסיליקון."],
    ["vga_r/g/b_int", "מחרוזות תווים וירטואליות.", "אכיפה חשמלית: חיווט הפינים לאדמה (GND) למניעת זליגות צבע אנלוגיות."]
]
